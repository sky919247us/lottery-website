...
- ⚠️ **跑完匯入或影片同步一定要 `sudo systemctl restart lottery-api`**。
  `/summary` 與 `/games` 有 10 分鐘的程序內快取（`app/api/cache.py` 是模組級 dict），
  腳本是獨立行程寫 DB，API 不會知道要失效，不重啟就會有最長 10 分鐘看到舊資料。
  `/games/{gameId}` 沒快取，所以會出現「詳情頁有影片、列表頁沒有」的錯覺。
